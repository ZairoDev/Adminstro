...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const employee = (await Employees.findOneAndUpdate(
      { _id: auth.id },
      { $set: { "pips.$[pip].acknowledgedAt": acknowledgedAt } },
      {
        arrayFilters: [
          {
            "pip.acknowledgmentRequired": true,
            "pip.acknowledgedAt": null,
          },
        ],
        new: true,
      },
    ...
    console.error("Failed to acknowledge current employee PIPs:", error);
    return NextResponse.json(
      { error: "Failed to acknowledge PIP information" },
      { status: 500 },
    );
  }
}