...
import { scheduleShortTermLocationGeoSync } from "@/services/unregistered-owner-geocode";
...
export async function PUT(
  ...
  try {
    ...
    const data = await unregisteredOwnerShortTerm.findById(params._id);
    if (!data) {
    ...
    if (GEO_TRIGGER_FIELDS.has(field)) {
      scheduleShortTermLocationGeoSync(data._id, {
...
export async function DELETE(
  ...
  const { _id } = params;
  try {
    await unregisteredOwnerShortTerm.findByIdAndDelete(_id);
    return NextResponse.json(
