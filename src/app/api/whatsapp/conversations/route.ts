...
import { syncPhoneConfigsWithMeta, fetchPhoneNumbersFromMeta, mapMetaPhonesToConfigs } from "@/lib/whatsapp/phoneMetadataSync";
  ...
  try {
    ...
    if (archivedOnly) {
      ...
      if (archivedConversationIds.length === 0) {
        // Fetch phone numbers directly from Meta API
        const businessAccountId = "2394287697653581";
        const metaPhones = await fetchPhoneNumbersFromMeta(businessAccountId);
        
        // Create area mapping from existing configs
        ...
        const allowedPhoneConfigs = filterPhoneConfigsByRole(allMetaConfigs, userRole, userAreas);
    ...
    const businessAccountId = "2394287697653581"; // Your WhatsApp Business Account ID
    const metaPhones = await fetchPhoneNumbersFromMeta(businessAccountId);
