...
import { generateBookingPdf, generateInvoicePdf } from "../../invoice/components/invoice-pdf";
import type { InvoiceData, ComputedTotals } from "../../invoice/page";
...
export default function DownloadPdfDialog({
  ...
  checkOut,
  existingGuests = [],
  existingRentPayable = 0,
  existingDepositPaid = 0,
}: Props) {
  const [loading, setLoading] = useState(false);
  const [open, setOpen] = useState(false);
  const [rentPayable, setRentPayable] = useState<number>(existingRentPayable);
  const [depositPaid, setDepositPaid] = useState<number>(existingDepositPaid);
  ...
  const [owners, setOwners] = useState<Owner[]>(
    existingGuests.length > 0
      ? existingGuests
      : [{ name, email: initialEmail, phoneNo: phone, idNumber: "", documents: [] }]
  );
  ...
  const handleGeneratePdfs = async () => {
    ...
    try {
      ...
      const invoiceData: InvoiceData = {
        name: owners[0]?.name || name,
        email: owners[0]?.email || email,
        phoneNumber: owners[0]?.phoneNo || phone,
        address: address || "",
        amount,
        sgst: 0,
        igst: 0,
        cgst: 0,
        totalAmount: amount,
        status: amountReceived >= amount ? "paid" : "unpaid",
        ...
        invoiceNumber: bookingId ? `ZI-${bookingId.slice(-5)}` : "ZI-XXXXX",
        sacCode: 9985,
      ...
      generateInvoicePdf(invoiceData, computed);
  ...
  return (
    <div className="flex flex-col gap-2">
      <Button onClick={() => setOpen(true)} disabled={loading} size="sm">
