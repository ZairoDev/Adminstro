...
export async function generateBookingPdf(data: {
  ...
  // === Booking Details ===
  drawLabelValue("Booking ID", data.booking_Id ?? "-");
  drawLabelValue("Total Amount ", `€${data.amount}`);
  drawLabelValue(
    "Paid Amount ",
    `€${data.finalPrice?.toFixed(2) ?? "0.00"}`
  );
  if (data.rentPayable !== undefined) {
    drawLabelValue("Rent Payable", `€${data.rentPayable.toFixed(2)}`);
  }
  if (data.depositPaid !== undefined) {
    drawLabelValue("Deposit Paid", `€${data.depositPaid.toFixed(2)}`);
  }
  drawLabelValue(
    "Number of People",
    data.numberOfPeople ? String(data.numberOfPeople) : "-"
  );
  drawLabelValue("Description", data.description || "-");
  drawLabelValue("Property Owner", data.propertyOwner || "-");
  drawLabelValue("Address", data.address || "-");
  drawLabelValue("Check-in", data.checkIn || "-");
  drawLabelValue("Check-out", data.checkOut || "-");
  y -= 20;
  ...
  link.href = url;
  link.download = `booking-${data.booking_Id || "details"}.pdf`;
