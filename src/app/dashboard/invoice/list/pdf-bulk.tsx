...
import { COMPANY_INFO } from "../components/invoice-shared";
import { TDocumentDefinitions } from "pdfmake/interfaces";
import { InvoiceData } from "../page";

(pdfMake as any).vfs = (pdfFonts as any).vfs;

export function generateBulkPdfSingleFile(invoices: InvoiceData[]) {
  const content: any[] = [];

  invoices.forEach((value, index) => {
    const isPaid = value.status === "paid";

    if (index > 0) content.push({ text: "", pageBreak: "before" }); // Page break between invoices
      ...
      columns: [
        ...
        },
        {
          stack: [
            ...
            { text: value.companyAddress || "-", fontSize: 12 },
          ],
          alignment: "right",
        },
      ],
      ...
      table: {
        ...
        body: [
          ...
          [
            ...
            { text: `€${value.sgst}`, alignment: "right" },
          ],
          [
            {},
            { text: "CGST:", alignment: "right" },
            { text: `€${value.cgst}`, alignment: "right" },
          ],
          [
            {},
            { text: "IGST:", alignment: "right" },
            { text: `€${value.igst}`, alignment: "right" },
          ],
          [
            {},
            { text: "Total:", bold: true, alignment: "right" },
            { text: `€${value.amount}`, bold: true, alignment: "right" },
          ],
        ],
      },
...
Branch: ${COMPANY_INFO.bank.branch}`,
        ...
        },
        {
          ...
          stack: [
            ...
            {
              text: `Support: ${COMPANY_INFO.company.supportEmail}`,
              fontSize: 10,
              margin: [0, 5, 0, 0],
            },
            {
