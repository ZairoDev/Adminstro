...
const EditDates = ({ params }: PageProps) => {
  ...
  const [isBlockDateDrawerOpen, setIsBlockDateDrawerOpen] =
  ...
  const fetchPropertyPrice = async () => {
    try {
      const response = await axios.post("/api/property/getPropertyById", {
        propertyId: params.id,
      });
      setPortionPrice(response.data.data.pricePerDay);
  ...
  const blockDates = () => {
    return (
      <Drawer
        open={isBlockDateDrawerOpen}
        onOpenChange={setIsBlockDateDrawerOpen}
      >
        <DrawerTrigger asChild>
          <Button variant="outline" className="flex gap-x-1">
            Block Dates <Ban size={16} className="text-xs" />
          </Button>
        </DrawerTrigger>
        <DrawerContent>
          <div className="mx-auto w-full max-w-sm">
            <DrawerHeader>
              <DrawerTitle>Block Dates</DrawerTitle>
              <DrawerDescription>
                Block the dates that are already booked
              </DrawerDescription>
            </DrawerHeader>
            <div className="flex items-center justify-center space-x-2">
              <DateRangePicker
                ...
                setDate={setDate}
                className="relative z-50"
                isDrawerOpen={isBlockDateDrawerOpen}
              />
            </div>
            <DrawerFooter>
              <Button onClick={handleBlockDates} className="hover:bg-white/60">
                Submit
              </Button>
              <DrawerClose asChild>
                <Button variant="outline">Cancel</Button>
  ...
  const handleChangePrice = async () => {
    try {
      const response = await axios.post("/api/property/editPrices", {
        propertyId: pId,
  ...
  const handleBlockDates = async () => {
    try {
      const response = await axios.post("/api/property/blockDatesInProperty", {
        propertyId: pId,
        dateRange: date,
      });
      toast({
      ...
      toast({
        variant: "destructive",
        title: "Error!",
        description: "Unable to block dates",
      });
    }
  };
