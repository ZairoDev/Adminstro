...
const Page = ({ params }: pageProps) => {
	...
	const [alreadyAddedProperty, setAlreadyAddedProperty] =
		useState<string>("");
	const [favouriteUpdatedProperties, setFavouriteUpdatedProperties] =
		useState<string[]>([]);
		...
		const fetchRoomProperties = async (roomId: string) => {
			try {
				const response = await axios.post(
					"/api/room/getPropertiesFromRoom",
					{
						roomId,
					}
				);
	...
	const removeProperty = async (index: number, propertyId: string) => {
		...
		try {
			const response = await axios.patch(
				"/api/room/removePropertyFromRoom",
				{
					roomId: roomId,
	...
	const debouncedAddToFavourite = useCallback(
		debounce(async (favouriteProperties: string[]) => {
			try {
				const response = await axios.patch(
					"/api/room/addPropertyToFavourite",
					{
						roomId,
