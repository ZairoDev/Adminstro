...
export interface PeopleEmployeeTableProps {
...
export default function EmployeeTable({
  ...
  const showOrgColumn = ["SuperAdmin", "HR"].includes(role);

  const regeneratePassword = async (employee: EmployeeRow) => {
  ...
  useEffect(() => {
    const visible = applyVisibilityFilters(employees, role, showInactive);
    setEmployeeList(visible);
    setFilteredEmployee(visible);
  }, [employees, role, showInactive]);
  ...
  return (
    <div className={showLockControls ? " w-full mt-2" : " w-full "}>
        ...
        <div className=" flex gap-x-2">
          {role !== "LeadGen-TeamLead" && (
            ...
            >
              ...
              <SelectContent>
                {["All", ...employeeRoles].map((roleOption) => (
      ...
      <Table>
        <TableHeader>
          <TableRow>
            ...
            {showLockControls && <TableHead>Status</TableHead>}
          ...
          {filteredEmployee.map((employee, index) => {
            ...
            return (
              <TableRow key={employee._id}>
                ...
                <TableCell>
                  <Badge
                    variant={employee.rentalType ? "outline" : "secondary"}
                  >
                  ...
                  <TableCell>
                    {hasOverdueActivePIP(employee) && employee.isLocked ? (
                      <div
                        ...
                        onClick={() => {
                          const isLevel3 = (employee.pips || []).some(
                            (p) =>
                              p.pipLevel === "level3" &&
                              p.status === "active" &&
                      ...
                      <ToggleButton
                        ...
                        onChange={async (value) => {
                          ...
                          try {
                            ...
                            if (value === true) {
                              try {
                                await axios.post("/api/employee/forceLogout", {
                                  employeeId: employee._id,
                                });
                                toast({
                                  title: "Employee force-logged out",
                                  description: `${employee.name} has been logged out.`,
                                });
                              } catch (err: unknown) {
                                ...
                                toast({
                                  title: "Warning",
                                  description:
                                    "Employee locked but force-logout failed. They will be logged out on next request.",
                                  variant: "destructive",
                                });
                              }
                            }
                            ...
                            console.error(
                              "Failed to update isLocked status",
                              error
                            );
                          }
                        }}
                    ...
                    <>
                      ...
                      {showPasswordRegen && (
                        <RefreshCcw
                          ...
                          onClick={() => void regeneratePassword(employee)}
                        />
                      )}
                    </>
                  )}
